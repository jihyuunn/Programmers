...
### 2-x   
- 가장 큰 정사각형   
- 처음에 bfs로 풀려고 했지만 정사각형이 겹쳐서 있을 경우 해결을 못함 또한 시간초과 문제   
- 검색을 했더니 dp로 풀어야 했다.. 현재 i,j위에 i-1,j/i,j-1/i-1,j-1 세개까지 정사각형을 이루고 세개의 최소값+1을 하면 현재 정사각형의 최대길이가 나온다   
- [[1,0,0], [0,0,0]]이 1이 나오게 처리를 해줘야하고 0으로만 이루어진 경우 0이 나오게 처리를 해줘야한다    